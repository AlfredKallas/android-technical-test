...
class FavouritesScreenTest {
    ...
    @Test
    fun `when paging has no items then show EmptyScreen`() {
        // Arrange
    ...
    fun `when paging has items then show AlbumList`() {
        // Arrange
        val albums = listOf(
            AlbumUIModel(id = 1, albumId = 1, title = "Fav Album 1", thumbnailUrl = "", isFavourite = true),
            AlbumUIModel(id = 2, albumId = 1, title = "Fav Album 2", thumbnailUrl = "", isFavourite = true)
        )
